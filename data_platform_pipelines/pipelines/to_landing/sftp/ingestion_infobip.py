...
from data_platform_pipelines.pipelines.utils.sftp.infobip_collector import (
    InfobipCollector,
)
...
def collect_data(
    ...
    **credentials,
    ...
    collector = InfobipCollector(**credentials)
    ...
    if widgets["incremental"] and about_ingestion["incremental_col"] != "-":
        ...
        n_days = (
            datetime.now()
            - datetime.strptime(last_update, "%Y-%m-%dT%H:%M:%S.000Z")
        ).days - 1
    else:
        logging.info(
    ...
    df = collector.start(
        ...
        keyword=about_ingestion["keyword"],
...
def main():
    ...
    credentials = SecretsManager().get_secrets(secret_name="infobip")
